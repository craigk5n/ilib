...
IError _IReadBMP ( FILE *fp, IOptions options, IImageP **image_return )
{
  ...
  if ( !ReadLong ( fp, &scratch ) || scratch != 40 )
    goto fail;
  ...
  if ( !ReadLong ( fp, &h ) )
    goto fail;
  if ( !ReadShort ( fp, &scratch ) || scratch != 1 )
    goto fail;
  if ( !ReadShort ( fp, &depth ) )
    goto fail;

  /*fprintf(stderr, "image size: %dx%dx%d\n", w, h, depth);*/

  if ( !ReadLong ( fp, &compression ) )
    goto fail;
  if ( !ReadLong ( fp, &imagesize ) )
    goto fail;
  if ( !ReadLong ( fp, &xpelspermeter ) )
    goto fail;
  if ( !ReadLong ( fp, &ypelspermeter ) )
    goto fail;
  if ( !ReadLong ( fp, &colorsused ) )
    goto fail;
  if ( !ReadLong ( fp, &colorsimportant ) )
    goto fail;
  ...
  */
  ...
  if ( depth <= 8 ) {
    ...
    if ( !colorsused )
      colorsused = 2 << depth;
    if ( colorsused < 0 || colorsused > ( 2 << depth ) )
      goto fail;
    ...
    for ( scratch = 0; scratch < colorsused; scratch++ ) {
      if ( !ReadLong ( fp, ( colortable + scratch ) ) )
        goto fail;
    }
  }
